...
const ConexionMP = () => {
  ...
  useEffect(() => {
    ...
    const decodeToken = () => {
      try {
        ...
        console.log("ID de usuario decodificado:", decoded.id); // Depuración
      } catch (error) {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      window.location.href = oauthUrl; // Redirige al usuario a Mercado Pago
    } catch (error) {
      console.error("Error en conectar con Mercado Pago:", error.response?.data || error.message);
      setError(
        error.response?.data?.error || "Error al iniciar la conexión con Mercado Pago. Verifica los logs."
      );
    } finally {
      setLoading(false);
    }
          ...
          <Card sx={styles.card}>
            <CardContent sx={{ textAlign: "center", p: 4 }}>
              ...
              <Typography variant="body1" sx={{ color: "#6c757d", mb: 3, lineHeight: 1.5 }}>
                Conecta tu cuenta de Mercado Pago para recibir pagos directamente en tu cuenta. Serás redirigido a Mercado Pago para autorizar la conexión de forma segura.
              </Typography>
              <Button
                variant="contained"
