...
const GestionReservas = () => {
  ...
  useEffect(() => {
    ...
    const fetchReservas = async () => {
      try {
        setIsLoading(true);
        const response = await axios.get("https://backendd-q0zc.onrender.com/api/gestionreservas/", {
  ...
  const fetchReservasByDate = async (date) => {
    try {
      setIsLoading(true);
      const response = await axios.get(`https://backendd-q0zc.onrender.com/api/gestionreservas/fecha/${date}`, {
